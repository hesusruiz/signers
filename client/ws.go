...
func (c *webSocketClient) subscribeChainHead(ch chan<- types.RawHeader) error {
	...
	log.Info("Send subscribe chain head message", "msg", msg)
...
func (c *webSocketClient) sendRPCMsg(ch chan<- *message, method string, args ...interface{}) error {
	...
	log.Info("Send JSON RPC message", "msg.Method", msg.Method, "args", args, "msg.ID", msg.ID)
	...
	for {
		...
		if c.conn == nil {
			if err := c.dial(c.rawUrl); err != nil {
				...
				log.Info("Retry connection in 1 second")
			...
			if c.chainHeadSubscriptionId != "" {
				if err := c.subscribeChainHead(c.chainHeadChan); err != nil {
					log.Info("Reconnect subscribe to chain head failed")
					c.resetConn()
					continue
				}
			}
		...
		log.Info("WebSocket message received", "msg", string(msg))
...
func (c *webSocketClient) resetConn() {
	log.Info("Reset WebSocket connection")
	// reset connection
	c.connMux.Lock()
	c.conn.Close()
	c.conn = nil
